...
function resolvePackageAsset(...segments) {
  return path.join(PACKAGE_ROOT, ...segments);
}

function defaultNoteTargets() {
  return {
...
function defaultLaunchTemplates() {
  return {
    open: null,
    reveal: null
  };
...
function normalizeVaultPath(vaultPath, cwd = process.cwd()) {
  if (!vaultPath) {
    return null;
  }
...
function isoDateInSeoul(date = new Date()) {
...
async function launchObsidianTarget(spec, runner = execFileAsync) {
