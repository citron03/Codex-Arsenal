...
7. Wait for the registry, then smoke test: `npx <package-name>@<version> --help`. See Post-Publish Verification; a stale version before the wait is not a failed publish.
...
A successful publish is not immediately visible. npm prints "Your package is being processed and may take a few minutes to become available", and a publish that signs provenance takes longer still. Observed propagation for a small package with provenance: 75s and 162s on two releases of the same package.
...
Wait for the registry before touching npm tooling:

```bash
until curl -s https://registry.npmjs.org/<package-name> | grep -q '"latest":"<version>"'; do sleep 10; done
```

Then smoke test:

```bash
npx <package-name>@<version> --help
```

The order matters, and not because there are two independent delays to sit through. There is one delay, in the registry. Running `npm view` or `npx` during that window makes a second problem: the client caches the packument it just fetched, which does not contain the new version, and keeps serving it after the registry is correct. That cached packument is what produces `ETARGET No matching version found` for a version that is demonstrably live.

So `--prefer-online` is the recovery, not the routine:

```bash
npx --prefer-online <package-name>@<version> --help
```

If the wait comes first, a plain `npx` works and the flag is unnecessary. If an impatient check came first, every later call needs the flag until the cache expires.
...
| `npm view` still reports the previous version after a successful publish | Registry metadata has not propagated yet | Poll the registry over HTTP and wait; never republish or bump the version again |
| `ETARGET No matching version found` for a version that is live on the registry | An `npm view` or `npx` run during the propagation window cached a packument without the new version | Retry with `npx --prefer-online` or `npm install --prefer-online`; avoid it next time by waiting on the registry before running npm tooling |
...
- Never run `npm view` or `npx` during the propagation window; the stale packument it caches is what causes `ETARGET` later.
